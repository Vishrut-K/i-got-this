...
export default function LoginPage() {
    ...
    try {
      if (isSignUp) {
        const { error } = await signUp.email({ 
            email, 
            password, 
            name: email.split("@")[0] // Default name for now
        });
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-background text-foreground overflow-hidden px-6">
      ...
      {/* Main Content */}
      <main className="w-full max-w-[420px] flex flex-col z-10 pt-8 pb-16">
        
        {/* Logo */}
        <div className="w-full flex justify-center mb-10 animate-fade-up delay-0">
        ...
        <div className="text-center mb-10">
          <h1 className="text-[28px] font-serif tracking-tight text-stone-900 dark:text-stone-100 animate-fade-up delay-100">
            {isSignUp ? "Begin your story." : "Welcome back."}
          </h1>
          <p className="text-stone-500 mt-3 animate-fade-up delay-200">
            {isSignUp ? "Your future self is waiting." : "Continue becoming."}
          </p>
        </div>
        ...
        <div className="animate-fade-up delay-300">
          <button
            ...
            className="w-full h-12 flex items-center justify-center gap-3 bg-white dark:bg-[#1C1B1A] border border-stone-200 dark:border-stone-800 rounded-[12px] text-stone-900 dark:text-stone-100 font-medium text-[15px] hover:bg-stone-50 dark:hover:bg-stone-900 transition-colors shadow-sm disabled:opacity-50"
          >
            {isGoogleLoading ? (
              <span className="animate-pulse">Continuing...</span>
            ) : (
              <>
                <svg viewBox="0 0 268.152 273.883" className="w-[18px] h-[18px]">
                  <defs>
        ...
        {/* Divider */}
        <div className="flex items-center my-6 animate-fade-up delay-400">
        ...
        {/* Email & Password Form */}
        <div className="flex flex-col gap-6 animate-fade-up delay-500">
          
          <div className="flex flex-col">
            <label className="text-xs font-medium text-stone-500 mb-2">Email</label>
            <input
              ref={emailRef}
              type="email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full bg-transparent border-b border-stone-200 dark:border-stone-800 py-2 text-stone-900 dark:text-stone-100 placeholder:text-stone-300 dark:placeholder:text-stone-700 outline-none focus:border-stone-400 dark:focus:border-stone-600 transition-colors"
              placeholder=""
          ...
          <div className="flex flex-col relative">
            <label className="text-xs font-medium text-stone-500 mb-2">Password</label>
            <div className="relative">
              <input
                type={showPassword ? "text" : "password"}
                value={password}
                onChange={(e) => setPassword(e.target.value)}
                className="w-full bg-transparent border-b border-stone-200 dark:border-stone-800 py-2 text-stone-900 dark:text-stone-100 placeholder:text-stone-300 dark:placeholder:text-stone-700 outline-none focus:border-stone-400 dark:focus:border-stone-600 transition-colors pr-8"
                placeholder=""
                onKeyDown={(e) => e.key === "Enter" && handleEmailAuth()}
              />
              <button 
          ...
          <button
            ...
            className="w-full h-12 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-[12px] font-medium hover:bg-stone-800 dark:hover:bg-white transition-colors shadow-sm disabled:opacity-50 mt-2"
          >
            {isLoading ? (
              <span className="animate-pulse">Continuing...</span>
            ) : (
          ...
          <div className="flex justify-center mt-2">
            <button 
              ...
              className="text-sm font-medium text-stone-500 hover:text-stone-800 dark:hover:text-stone-300 transition-colors"
            >
      ...
      {/* Footer */}
      <footer className="absolute bottom-8 w-full text-center z-10 animate-fade-up delay-500">
